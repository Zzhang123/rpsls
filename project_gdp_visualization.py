...
def reconcile_countries_by_name(plot_countries,gdp_countries): #返回在世行有GDP数据的绘图库国家代码字典，以及没有世行GDP数据的国家代码集合
	...
	Set=set()
	Tup=(Dict,Set)
	for word in plot_countries:
		for x in gdp_countries.values():
			if plot_countries[word]==x["Country Name"]:
				for years in range(1960,2015):
					if x[str(years)]=="":
						continue
					else:
						Dict[word]=x
...
def build_map_dict_by_name(gdpinfo,plot_countries, year):
	...
	with open(gdpinfo["gdpfile"],"rt") as csvfile:
		...
		for cow in reader:
			for word in plot_countries:
				country=plot_countries[word]
				if country==cow[gdpinfo["country_name"]] and cow[year]!="":
					Dict[word]=math.log10(float(cow[year]))
